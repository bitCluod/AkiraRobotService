...
def runnav(mapname,init_pose):
	...
	#time.sleep(3)
	send_command('cd /home/damocles/Documentos/Akira_1.2.1/marrtino_apps/navigation','loc')
	params = 'map_name:=%s initial_pose_x:=%.2f initial_pose_y:=%.2f initial_pose_a:=%.2f' %(mapname,init_pose[0],init_pose[1],init_pose[2]/180*math.pi)
	send_command('roslaunch srrg_localizer.launch %s' %params,'loc')
	time.sleep(3)
	send_command('cd /home/damocles/Documentos/Akira_1.2.1/marrtino_apps/navigation','nav')
