...
#define FOLLOW_CORRIDOR_TOPIC_NAME "follow_corridor"
...
void fc_action_client(std::string robot_name, double GX, double GY, double maxvel) {

    // Set fc topic
    std::string fc_topic = "/"+robot_name+"/"+FOLLOW_CORRIDOR_TOPIC_NAME;
    ...
    while (!ac.waitForResult(ros::Duration(1.0))) {
	  ...
	  ROS_INFO("Follow Corridor R:(%.1f,%.1f) -> G:(%.1f,%.1f)",RX,RY,GX,GY);
	  // ROS_INFO_STREAM("Follow Corridor... [" << ac.getState().toString() << "]");
    }
...
// Set fc topic
    std::string fc_topic = "/"+robot_name+"/fc";
...
int main(int argc, char** argv)  {
  ...
  if ( (argc<2) || ((std::string(argv[1])=="-client") && (argc<4)) ) {
  	std::cout << "Use: " << argv[0] << " -server <robot_name>" << std::endl; 
  	std::cout << "  or " << argv[0] << " -client <robot_name> [<X> <Y> <maxvel>|stop] " << std::endl; 
    exit(-1);
  }
  ...
  if (std::string(argv[1])=="-server") {
      ...
      // Init ROS node
      std::string node_name = std::string(FOLLOW_CORRIDOR_TOPIC_NAME)+std::string("_action_server");
      ros::init(argc, argv, node_name.c_str());
      // Start action server  
      FollowCorridorActionServer server(FOLLOW_CORRIDOR_TOPIC_NAME,robot_name);
      server.start();
      ros::spin();
  }
      ...
      if (std::string(argv[3])=="stop") {
        // Init ROS node
	      ...
	      ss << FOLLOW_CORRIDOR_TOPIC_NAME << "_" << robot_name << "_stop";
	      ...
	      double GX = atof(argv[3]), GY = atof(argv[4]), maxvel = atof(argv[5]);
	      // Init ROS node
	      std::ostringstream ss;
	      ss << FOLLOW_CORRIDOR_TOPIC_NAME << "_" << robot_name << "_" << (int)(fabs(GX)) << "_" << (int)(fabs(GY));
	      std::string nodename = ss.str();
	      ros::init(argc, argv, nodename);
	      // Start client
	      fc_action_client(robot_name,GX,GY,maxvel);
      }
  }
  return 0;
}
