...
CIMUMessageListener::CIMUMessageListener(  ros::NodeHandle* p_pNode, txt_io::SensorMessageSorter* p_pSorter ): m_pNode( p_pNode ), m_pSorter( p_pSorter )
{
    //ds nothing to do
}
...
void CIMUMessageListener::_callbackIMU( const sensor_msgs::ImuPtr p_pIMUMessage )
{
    ...
    txt_io::CIMUMessage* msgIMU = new txt_io::CIMUMessage( m_strTopic, p_pIMUMessage->header.frame_id,  p_pIMUMessage->header.seq, p_pIMUMessage->header.stamp.toSec( ) );
