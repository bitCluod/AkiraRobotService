...
Map_names = ['None','DISB1','DISLabs'] 
Demo_names = ['None','joystick','move_base/rviz'] 

def run_demo(mapname, robotname, demo):
  print 'Running demo: map:%s robot:%s demo:%s' % (mapname,robotname, demo)
  if (demo=='joystick'):
...
class GUI(tk.Frame):
   ...
   def launch_script(self):
      ...
      thread.start_new_thread( run_demo, (self.map_ddm.get(), 'diago', self.demo_ddm.get()) )
