...
namespace ros_wrappers {
  ...
  void SphericalImageMessageListener::subscribe(const std::string& topic_, int num_messages) {
    ...
    _subscriber = _node_handle->subscribe<thin_msgs::SphericalDepthImage>(_topic, 100, &SphericalImageMessageListener::imageCallback, this);
    cerr << "subscribing to topic: " << _topic  << endl; 
  }


  void SphericalImageMessageListener::imageCallback(const thin_msgs::SphericalDepthImageConstPtr& msg) {
